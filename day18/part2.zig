...
fn compute(alloc: std.mem.Allocator, lhs: []Token, rhs: []Token) !i32 {
    var l = try L.init(alloc, lhs);
    defer l.deinit();
    try l.add(rhs);
    while (try l.explode() or try l.split()) {}
    try l.reduce();
    return switch (l.l.items[0].*) {
        .Number => |n| n,
        else => unreachable,
    };
}
...
pub fn solve(alloc: std.mem.Allocator, data: []const u8) !void {
    ...
    var res: i32 = std.math.minInt(i32);
    for (l.items, 0..) |l1, i| {
        for (l.items, 0..) |l2, j| {
            if (i != j) res = @max(res, try compute(alloc, l1, l2));
        }
    }
