...
pub fn solve(alloc: std.mem.Allocator, data: []const u8) !void {
    var newData = std.ArrayList(u8).init(alloc);
    defer newData.deinit();
    {
        var lines = std.ArrayList([]const u8).init(alloc);
        defer lines.deinit();
        var it = std.mem.tokenizeScalar(u8, data, '\n');
        while (it.next()) |line| try lines.append(line);
        try lines.insert(3, "  #D#C#B#A#");
        try lines.insert(4, "  #D#B#A#C#");
        for (lines.items) |line| {
            try newData.appendSlice(line);
            try newData.append('\n');
        }
    }
